...
const calculateAtsScore = (parsedResumeText, parsedStructure, jobDescriptionText, jobRequiredSkills = []) => {
    let totalScore = 0;
    // 1. Section Completeness (Max 20 points)
    let sectionCompleteness = 0;
    if (parsedStructure.contact?.email)
        sectionCompleteness += 5;
    if (parsedStructure.education && parsedStructure.education.length > 0)
        sectionCompleteness += 5;
    if (parsedStructure.experience && parsedStructure.experience.length > 0)
        sectionCompleteness += 5;
    if (parsedStructure.skills && parsedStructure.skills.length > 0)
        sectionCompleteness += 5;
    ...
    if (jobRequiredSkills.length > 0) {
        const resumeSkills = (parsedStructure.skills || []).map((s) => s.toLowerCase());
        const matchedSkills = jobRequiredSkills.filter(reqSkill => resumeSkills.includes(reqSkill.toLowerCase()) || parsedResumeText.toLowerCase().includes(reqSkill.toLowerCase()));
