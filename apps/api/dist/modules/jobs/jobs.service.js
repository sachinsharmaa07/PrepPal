...
class JobsService {
    async getJobs(filters) {
        const { status = 'PUBLISHED', page = 1, limit = 20, tenure, location, skills, minSalary, maxSalary, isRemote, sortBy = 'createdAt', } = filters;
        ...
        const orderBy = sortBy === 'salary_desc'
            ? { salaryMax: 'desc' }
            : sortBy === 'salary_asc'
                ? { salaryMin: 'asc' }
                : { createdAt: 'desc' };
        const [jobs, total] = await db_1.default.$transaction([
            db_1.default.job.findMany({
                where,
                skip,
                take: Number(limit),
                orderBy,
